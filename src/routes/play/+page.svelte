<script>
  ...
  import { theme, toggleTheme as toggleThemeStore } from '$lib/stores/theme';
  ...
  function draw(ctx) {
    ...
    game.platforms.forEach(platform => {
      ...
      // Grass top (3 layers)
      const grassTop = $theme === 'dark' ? '#1a5a10' : '#3a8a1a';
      const grassMid = $theme === 'dark' ? '#2a7a1a' : '#5aaa2a';
      const grassTip = $theme === 'dark' ? '#38a020' : '#70cc30';
  ...
  onMount(() => {
    // Redirect touch / narrow devices to the CV page — the platformer needs
    // a keyboard and a wide viewport.
...
{#if !$isLoading}
<div class="container" style="--bg:{themes[$theme].bg}; --panelBg:{themes[$theme].panelBg}; --cardBorder:{themes[$theme].cardBorder}; --cardText:{themes[$theme].cardText}; --cardPeriod:{themes[$theme].cardPeriod}; --cardH3:{themes[$theme].cardH3}; --buttonBg:{$theme === 'light' ? 'rgba(216, 200, 154, 0.75)' : 'rgba(0,0,0,0.6)'};">
  <div class="game-wrapper">
    <canvas bind:this={canvas}></canvas>
    <a class="back-to-cv" href="/" aria-label={$_('cv.cta.backToCV')}>
      <span class="back-arrow">←</span> {$_('cv.cta.backToCV')}
    </a>
